...
import com.eazybyte.cards.constants.CardsConstants;
...
@Tag(
        ...
        description = "CRUD REST APIs in Bank to CREATE,FETCH,UPDATE AND DELETE cards details"
)
@RestController
...
public class CardsController {
    ...
    @Operation(
            ...
            description = "REST API to fetch a Card inside Bank"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
    ...
    @GetMapping("/fetch")
    public ResponseEntity<CardsDto> fetchCard(@RequestHeader("bank-correlation-id")String correlationId,
                                        @Valid @Pattern(regexp = "(^$|[0-9]{9})",message = "Mobile number must be 9 digits")
